...
## GAE Caveats for Snakr v1.0.1 (3/1/2016)

GAE's Python SDK includes a distro of Django 1.5.11 (later versions are not officially supported). This version of Snakr installs Django 1.9.2 on GAE 
but some functions won't work (those you'd expect around security, networking, etc. that would conflict with GAE's ecosystem). Note that Django 1.9.2 
is included in my lib folder, while 1.5.11 is in the GAE SDK external library. Behavior may not be what you expect in your dev and other environs 
as a result. Be advised.
