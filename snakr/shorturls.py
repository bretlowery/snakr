...
class ShortURL:
    ...
    def getlongurl(self, request):
        ...
        try:
            ...
            if not s:
                raise self.event.log(messagekey='SHORT_URL_NOT_FOUND', value=self.shorturl, status_code=400)
        except:
            raise self.event.log(messagekey='SHORT_URL_NOT_FOUND', value=self.shorturl, status_code=400)

        if s.shorturl != self.shorturl:
            raise self.event.log(messagekey='SHORT_URL_MISMATCH', status_code=400)
        #
        # If the short URL is not active, 404
        #
        if s.is_active != 'Y':
            raise self.event.log(messagekey='HTTP_404', value=self.shorturl, status_code=404)
        #
